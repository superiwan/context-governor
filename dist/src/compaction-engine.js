...
export class CompactionEngine {
    auditor;
    constructor(auditor = new SummaryAuditor()) {
        this.auditor = auditor;
    }
    compact(messages, config, injectedSections, memoryState) {
        ...
        const audit = this.auditor.audit(summary, memoryState.todos.filter((item) => item.status === "active"));
        return {
            summary,
            preservedTurns,
            injectedSections,
...
function buildTaskSummary(messages, memoryState) {
...
function pickLines(memoryRecords, messages, tag) {
...
function flattenMemoryState(memoryState) {
    return [
        ...memoryState.goals,
