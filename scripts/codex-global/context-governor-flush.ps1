...
try {
    Append-EventLog -MemoryDir $MemoryDir -SessionId $sessionId -WorkspacePath $resolvedWorkspace -ThreadId $resolvedThreadId -EventType "flush"
    node .\dist\src\cli.js flush --session $sessionId --memoryDir $MemoryDir
}
finally {
    Pop-Location
}
