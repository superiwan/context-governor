...
try {
    Append-EventLog -MemoryDir $workspaceMemoryDir -SessionId $sessionId -WorkspacePath $resolvedWorkspace -ThreadId $resolvedThreadId -EventType "resume"
    node .\dist\src\cli.js resume --session $sessionId --memoryDir $workspaceMemoryDir
}
finally {
    Pop-Location
}
