...
try {
    Append-EventLog -MemoryDir $MemoryDir -SessionId $sessionId -WorkspacePath $resolvedWorkspace -ThreadId $resolvedThreadId -EventType "compact"
    node .\dist\src\cli.js compact --session $sessionId --memoryDir $MemoryDir
}
finally {
    Pop-Location
}
