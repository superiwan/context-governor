...
try {
    Append-EventLog -MemoryDir $workspaceMemoryDir -SessionId $sessionId -WorkspacePath $resolvedWorkspace -ThreadId $resolvedThreadId -EventType "compact"
    node .\dist\src\cli.js compact --session $sessionId --memoryDir $workspaceMemoryDir
}
finally {
    Pop-Location
}
